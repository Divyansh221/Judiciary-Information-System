...
passwo = input("Enter your password for your SQL server: ")		# Input your MySQL server password
...
mycursor.execute("CREATE DATABASE JIS_Database")		# Creating new database to store information in tables

myDB = mysql.connector.connect(
  host = "localhost",
  user = "root",
  password = passwo,						# Connecting to local MySQl server
...
mycursor.execute("ALTER TABLE users ADD COLUMN name VARCHAR(255)")		# Creating User information table
...
mycursor.execute("ALTER TABLE cases ADD COLUMN defendentAddress VARCHAR(255)")		# Creating Cases information table
...
mycursor.execute("ALTER TABLE slots ADD COLUMN slot1 BIT(1) DEFAULT 0")			# Creating Slots information table
...
print("Database created succesfully.")		# Printing success message
print("Also, check out Database.py in Databases folder. Change the password in it to your password.")
print("There are two passwords in there btw.")		# Additional instruction to be followed
